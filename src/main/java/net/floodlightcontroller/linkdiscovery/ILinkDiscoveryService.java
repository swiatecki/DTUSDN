...
public interface ILinkDiscoveryService extends IFloodlightService {
	...
	/**
	 * Returns OFPacketOut which contains the LLDP data corresponding to
	 * switchport (sw, port). PacketOut does not contain actions. PacketOut
	 * length includes the minimum length and data length.
	 */
	 ...
	 * certain packets early in the packet-in processing chain. Since
	 * LinkDiscovery is first in the packet-in processing pipeline, it can
	 * efficiently drop these packets. Currently these packets are identified
	 * only by their source MAC address.
	 * 
	 * Add a MAC address range to ignore list. All packet ins from this range
	 * will be dropped - use with care!
	 * 
	 * @param mac
	 *            The base MAC address that is to be ignored
	 * @param ignoreBits
	 *            The number of LSBs to ignore. A value of 0 will add only one
	 *            MAC address 'mac' to ignore list. A value of 48 will add ALL
	 *            MAC addresses to the ignore list. This will cause a drop of
	 *            ALL packet ins.
	 */
