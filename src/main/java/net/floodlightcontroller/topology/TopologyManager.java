...
import net.floodlightcontroller.topo_try.ITopologyInstanceService;
...
public class TopologyManager implements IFloodlightModule, ITopologyService, IRoutingService, ILinkDiscoveryListener,
		IOFMessageListener, ITopologyInstanceService {
	...
	protected class TopologyEventInfo {
		...
		public TopologyEventInfo(int numOpenflowClustersWithTunnels, int numOpenflowClustersWithoutTunnels,
				Map<Long, List<NodePortTuple>> externalPortsMap, int numTunnelPorts) {
			super();
	...
	@Override
	public Route getRoute(long src, short srcPort, long dst, short dstPort, long cookie, boolean tunnelEnabled) {
		TopologyInstance ti = getCurrentInstance(tunnelEnabled);
	...
	private class HAListenerDelegate implements IHAListener {
		...
		public void transitionToMaster() {
			role = Role.MASTER;
			log.debug("Re-computing topology due " + "to HA change from SLAVE->MASTER");
			newInstanceTask.reschedule(TOPOLOGY_COMPUTE_INTERVAL_MS, TimeUnit.MILLISECONDS);
		}
	...
	public Map<Class<? extends IFloodlightService>, IFloodlightService> getServiceImpls() {
		...
		m.put(ITopologyInstanceService.class, this);
		return m;
	}
	...
	protected void registerTopologyDebugEvents() throws FloodlightModuleException {
		...
		try {
			evTopology = debugEvents.registerEvent(PACKAGE, "topologyevent", "Topology Computation",
					EventType.ALWAYS_LOG, TopologyEvent.class, 100);
	...
	protected Command dropFilter(long sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (isAllowed(sw, port) == false) {
			if (log.isTraceEnabled()) {
				log.trace("Ignoring packet because of topology " + "restriction on switch={}, port={}", sw, port);
				result = Command.STOP;
			}
		}
		return result;
	...
	public void doMultiActionPacketOut(byte[] packetData, IOFSwitch sw, Set<Short> ports, FloodlightContext cntx) {
	...
	protected boolean createNewInstance(String reason) {
		...
		evTopology.updateEventWithFlush(new TopologyEvent(reason, topologyInfo));
		return true;
	}
	...
	protected Set<NodePortTuple> identifyBroadcastDomainPorts() {
		...
		for (NodePortTuple npt : spLinks.keySet()) {
			...
			if (links.size() > 2) {
				...
				if (l1.getSrc() != l2.getDst() || l1.getSrcPort() != l2.getDstPort() || l1.getDst() != l2.getSrc()
	...
	protected void addOrUpdateTunnelLink(long srcId, short srcPort, long dstId, short dstPort) {
	...
	public Set<Long> getSwitches() {
		return getCurrentInstance().getSwitches();
	}
